...
1. [为什么是 PKCE](#1-为什么是-pkce)
   - 1.1 Client Secret 无法安全存储
   - 1.2 Client Secret vs Token
   - 1.3 PKCE 方案
...
### 1.2 Client Secret vs Token：为什么 secret 泄露更严重

```
client_secret 泄露（PKCE 去掉了这个）：
  - 写死在代码里，每个用户的安装包里都有同一个 secret
  - 攻击者反编译一个包就拿到了
  - 可以用它冒充应用，去骗任何用户授权
  - 影响范围：所有用户，永久有效，除非微软吊销整个应用

access_token / refresh_token 泄漏（PKCE 也需要这些）：
  - 存在内存/safeStorage 里，不随安装包分发
  - 每个用户不同，泄露了只影响这一个用户
  - access_token 1 小时过期，refresh_token 可以单独吊销
  - 要泄露得攻破这台具体的机器
```

PKCE 的逻辑不是"token 不会泄露"，而是：

1. **去掉最大的单点故障**（client_secret 不再存在）
2. **缩小爆炸半径**（泄露一个用户的 token 比泄露整个应用的 secret 好得多）
3. **增加攻击门槛**（要偷 token 得先攻破本机，不能只靠反编译）

### 1.3 方案：PKCE (Proof Key for Code Exchange)
...
攻击者的目标：让用户的应用绑定到**攻击者的 OneDrive**，这样用户的数据就同步到攻击者那里了。

**第一步：攻击者提前准备**

```
攻击者自己打开微软登录，用攻击者自己的账号密码登录
    │
    ▼
微软给攻击者发了一个 code = ATTACKER_CODE
攻击者把这个 code 记下来
```
...
```
攻击者构造一个回调链接，把 code 换成自己的：
  superproductivity://oauth-callback/onedrive?
    code=ATTACKER_CODE     ← 攻击者伪造的
    state=???              ← 攻击者不知道该填什么
...
```
用户点击链接 → 应用收到回调，提取 code=ATTACKER_CODE, state=???
    │
    ▼
第一关 — State 验证：
    应用内存里存的 state = "随机值A"（用户点 Authorize 时生成的）
    回调里的 state = "???"（攻击者猜的/空的）
    → 不匹配！→ 拒绝！攻击结束 ✗
    │
    │（假设攻击者绕过了 state，比如应用实现有 bug）
    ▼
第二关 — PKCE 验证：
    应用用自己的 code_verifier="随机值B" 去微软换 token
    微软检查：SHA256("随机值B") == 攻击者的 code_challenge ?
    → 不匹配！→ 微软拒绝！攻击结束 ✗
```

**两层防护的纵深防御**：即使一层被绕过（比如实现有 bug），另一层还能挡住。

### 3.2 两层防护分别防什么

两层防的是**同一个攻击**（注入攻击者的 code），但在**不同的环节**挡：

| 层        | 检查的问题                          | 拒绝的理由                              |
| --------- | ----------------------------------- | --------------------------------------- |
| **State** | 这个回调，是我之前发起的那个吗？    | state 不在内存里 → "我没发起过这个请求" |
| **PKCE**  | 这个 code，能用我的 verifier 换吗？ | verifier 不匹配 → "这个 code 不是我的"  |

关键原理：两个随机值（state 和 code_verifier）都是在用户点击 Authorize 时生成的，存在应用内存里。攻击者只能在回调时动手，但这时候随机值已经存在了，他没法伪造。

```
时间线：
─────────────────────────────────────────────────────────
  ① 应用生成 state + code_verifier，存在内存里
  ② 用户跳转到微软登录页
  ③ 用户输入密码，授权
  ④ 微软回调，带回 code + state
  ⑤ 应用验证
...
攻击者只能在第④步拦截或注入，但第①步的值他拿不到。
```

### 3.3 什么时候两层都失效？

只有**完全控制应用本身**才行（恶意软件/恶意 Electron 插件），但这时候他本来就能做任何事了，不需要 OAuth 攻击。
...
OAuth 的 **refresh_token 只能用一次**。用完就作废，服务端会发一个新的回来。

```
没有单飞锁时：
  请求A: 调用 API → 401 → 开始刷新 token
  请求B: 调用 API → 401 → 也开始刷新 token（同一个 refresh_token）

  请求A: POST /token (refresh_token=旧的) → 成功，拿到新 token
  请求B: POST /token (refresh_token=旧的) → 失败！旧 refresh_token 已作废
       → 400 Bad Request → 用户被踢出去要重新登录
```

单飞锁的意思是：**第一个请求去刷新，其他请求等着**，不重复发刷新请求。

```
有单飞锁时：
  请求A: 发现没人在刷新 → 我来刷新 → 成功 → 更新 token
  请求B: 发现有人在刷新 → await 等待 → 刷新完了 → 用新 token 重试
```
...
### 6.4 为什么用 while 轮询而不是 Promise 缓存？
