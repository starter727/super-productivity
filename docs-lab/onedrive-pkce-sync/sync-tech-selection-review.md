...
### 3.4.1 补充说明：向量时钟 + LWW 的协作原理

**为什么不是直接 LWW？**

一个常见的疑问是：既然冲突最终都是靠 LWW（Last-Write-Wins，比较时间戳）裁决，那向量时钟是否多余？能否去掉向量时钟，所有操作都直接按时间戳 PK？

答案：**向量时钟的作用是大幅减少需要 LWW 裁决的场景，让 LWW 只作为最后的手段。** 如果去掉向量时钟，每一次对同一实体的两个操作都会被当作冲突处理——即使它们之间是因果关系，根本不冲突。

**向量时钟的四种比较结果**

在 `conflict-resolution.service.ts` 的 `compareVectorClocks` 方法中，两个向量时钟的比较产生四种结果，其中只有一种会触发 LWW：

| 比较结果         | 含义                                         | 处理方式          | 数据丢失？ |
| ---------------- | -------------------------------------------- | ----------------- | ---------- |
| **GREATER_THAN** | 本地时钟支配远程（本地已包含远程的所有更改） | 跳过远程操作      | 无         |
| **LESS_THAN**    | 远程时钟支配本地（远程已包含本地的所有更改） | 直接应用远程操作  | 无         |
| **EQUAL**        | 完全相同的操作（如重复同步）                 | 去重跳过          | 无         |
| **CONCURRENT**   | 两个客户端在互不知晓的情况下各自独立修改     | **触发 LWW 裁决** | 一方丢失   |

**实际场景对比**

考虑一个典型的多设备场景：用户在手机上修改任务标题 → 同步。5 分钟后在电脑上修改同一个任务的备注 → 同步。

- **只有 LWW（无向量时钟）**：两次操作修改了同一实体，系统不知道它们之间的因果关系。按时间戳比较，后发生的覆盖先发生的。**如果服务器接收顺序与时间戳顺序不一致（网络延迟、队列重排），标题修改可能被备注修改意外覆盖而丢失。**

- **向量时钟 + LWW**：第一个操作发生后，本地向量时钟推进。第二个操作继承了这个推进后的时钟。远端收到两个操作时，向量时钟比较结果为 LESS_THAN（第二个操作"看见"了第一个操作的效果），系统知道这是因果顺序的延续，两个操作都正常应用。**只有双方真的同时独立修改同一实体时（CONCURRENT），才用 LWW 时间戳决胜负。**

**在代码中的体现**

```typescript
// conflict-resolution.service.ts — compareVectorClocks 分类
// GREATER_THAN: 本地支配远程 → 跳过，零数据丢失
// LESS_THAN: 远程支配本地 → 应用，零数据丢失
// EQUAL: 相同操作 → 去重，零数据丢失
// CONCURRENT: 真正的冲突 → 进入 _resolveConflictsWithLWW (timestamp tie-break)
```

**结论**：向量时钟把"这是不是冲突"的判断从猜想变成数学推导。只有 CONCURRENT 操作才真正触发 LWW。在实践中，同一用户在多设备上的同实体操作绝大多数是顺序的（先后操作），而非真正并发——向量时钟将它们从 LWW 的"无差别覆盖"中拯救出来，避免了不必要的静默数据丢失。

### 3.4.2 补充说明：NgRx Store 是内存存储，断电会丢失吗？

**是的，NgRx Store 完全在内存中。** 它是 JavaScript 对象，不直接持久化。但这不意味着数据不安全——Super Productivity 有多层持久化机制：

```
用户操作 → NgRx Store（内存）
              │
              ├─→ IndexedDB（浏览器持久化存储，写入磁盘）
              │     └─ 应用退出/断电/崩溃后，下次启动时从此恢复
              │
              └─→ 操作日志（Operation Log）→ 加密 → 上传到 OneDrive/Dropbox
                    └─ 多设备同步 & 异地备份
```

**启动时的数据恢复流程**：

1. 应用启动 → 从 IndexedDB 读取上一次的完整 Store 状态 → 恢复到内存中的 NgRx Store
2. 如果启用了同步 → 从网盘下载 `sync-data.json` → 解密 → 合并操作日志（向量时钟去重 + LWW 冲突解决）→ 更新 NgRx Store
3. 每次 NgRx Store 中的持久化状态变更 → 自动写入 IndexedDB

**关于"双写"问题的澄清**

文档中多次出现的"双写"（dual-write）指的不是 NgRx → IndexedDB 的持久化——那是单向的、自动的、由 NgRx Effect 或 persistence service 保证一致。文档中的"双写"指的是：**如果同时使用 NgRx Store 和另一个状态系统（如 Yjs document、SQLite 数据库），两个状态源之间必须互相手动同步**。

这种双写的问题在于：

- **原子性缺失**：两个写入操作不能被包裹在同一个事务中。一个成功、一个失败 → 状态分裂
- **操作语义丢失**：Yjs/CRDT 只知道"数据变了"，不知道变的意图（是归档还是修改标题？）
- **冲突解决降级**：接收端收到的是数据 diff，不是操作意图，无法应用归档优先等业务规则

NgRx Store → IndexedDB 不构成双写问题，因为 IndexedDB 只是 Store 的持久化快照，不是独立状态源。真正的状态始终在 Store 中，IndexedDB 是只读的恢复数据源。
