...
上表中 R1–R9 来自 PR 过程中逐轮修复的迭代，R10 来自第二轮正式 review。这些问题在最终 merge 前全部被修复。此外，在 R1 和 R10（两轮正式 review）之间，还有 6 轮 PR conversation comment（C1–C6, 2026/05/20–05/23），每轮 reviewer 在 recheck 最新 commit 时提出了额外的发现。这些中间轮次的问题（#21–#39）在后续提交中被逐一修复，详见下方"中间轮次 Review 的额外发现"章节。
